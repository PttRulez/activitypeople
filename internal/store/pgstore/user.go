...
func (pg *UserPostgres) GetByIdWithStrava(ctx context.Context, id int) (*model.User, error) {
	...
	var strava model.StravaInfo
	var hasStrava *int
	var u model.User
	err := row.Scan(&u.Id, &u.Email, &u.HashedPassword, &u.Name, &u.Role, &strava.AccessToken, &strava.RefreshToken, &hasStrava)
	if hasStrava == nil {
		u.Strava = nil
	} else {
		u.Strava = &strava
